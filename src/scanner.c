...
static TokenType checkKeyword(int start, int length, const char* string, TokenType tokenType) {
    if((scanner.current-scanner.start == start + length) && memcmp(scanner.start + start, string, length) == 0) {
        return tokenType;
    }
